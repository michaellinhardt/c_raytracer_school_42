...
G_MODULE_EXPORT	gboolean	on_key_press(GtkWidget *widget, GdkEvent  *event, void *user_data)
{
	...
	if (s->to_move->type & COMPLEXE)
		...
		s->to_move->pos[2] += 1.0;
	else if (key->keyval == GDK_KEY_KP_3)
		s->view_angle[0] += 10;
	else if (key->keyval == GDK_KEY_KP_6)
		s->view_angle[1] += 10;
	else if (key->keyval == GDK_KEY_KP_9)
		s->view_angle[2] += 10;
	if ((key->keyval >= GDK_KEY_KP_1 && key->keyval <= GDK_KEY_KP_9) || key->keyval == GDK_KEY_Up || key->keyval == GDK_KEY_Down)
		raytracing(s);
	return 0;	
}
